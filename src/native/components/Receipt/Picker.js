...
class PickedUser extends Component {
  ...
  render() {
    ...
    return (
      <ScrollView>
      <View>
      {receipt.items.filter(item => (item.user_claim === ''))
      .map(itemObj => (
      <ListItem key={itemObj.id} rightIcon={{ style: { opacity: 0 } }}>
        <Picker
          style={{ width: 100 }}
          selectedValue={this.state.pickerVal}
          onValueChange={(value, label) => this.setState({ pickerVal: value, selectedUser: receiptUsers[value].props.label })}
        >
          {receiptUsers}
        </Picker>
        <Button
          onPress={() => assignUser(this.state.selectedUser, itemObj.id)}
        >
          <Icon>+</Icon>
        </Button>
        <Text>
